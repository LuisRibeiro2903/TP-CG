...
ParsedModel generatePatch(string &patchfile, int tesselationLevel) {
    ...
    for (auto patch : patches){
        ...
        for (float u = 1; u <= tesselationLevel; u++) {
            float pu = (u - 1) / static_cast<float>(tesselationLevel);
            float uVector[4] = {pu * pu * pu, pu * pu, pu, 1};
            float duVector[4] = {3 * pu * pu, 2 * pu, 1, 0};
            float next_pu = u / static_cast<float>(tesselationLevel);
            ...
            for (float v = 1; v <= tesselationLevel; v++) {
                float pv = (v - 1) / static_cast<float>(tesselationLevel);
                float vVector[4] = {pv * pv * pv, pv * pv, pv, 1};
                float dvVector[4] = {3 * pv * pv, 2 * pv, 1, 0};
                float next_pv = v / static_cast<float>(tesselationLevel);
                ...
                Point t1 (1 - (texStep * (v - 1)), 1 - (texStep * (u - 1)), 0);
                ...
                Point t2 (1 - (texStep * v), 1 - (texStep * (u - 1)), 0);
                ...
                Point t3 (1 - (texStep * (v - 1)), 1 - (texStep * u), 0);
                ...
                Point t4 (1 - (texStep * v), 1 - (texStep * u), 0);
