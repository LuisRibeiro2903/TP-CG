...
ParsedWorld *worldParser(const char *filename) {
  ...
  if (lightsElement) {
    for (tinyxml2::XMLElement *lightElement =
         ...
         lightElement = lightElement->NextSiblingElement("light"), lightId++) {
      ...
      case 's': {
        ...
        LightSpot *l =
            new LightSpot(posX, posY, posZ, dirX, dirY, dirZ, cutoff, lightId);
        lights.push_back(l);
        break;
      }
      }
  ...
  if (primaryGroupElement) {
    ...
    std::cerr << "Error: invalid file format (no 'group' element, must have at "
                 "least one)"
              << std::endl;
    exit(1);
  }

  return new ParsedWorld(lookAt, projection, windowWidth, windowHeight,
                         rootGroup, lights, lights.size());
}
...
GroupNode *ParseGroupElement(tinyxml2::XMLElement *groupElement) {
  ...
  tinyxml2::XMLElement *transformElement =
      groupElement->FirstChildElement("transform");
  if (transformElement) {
    for (tinyxml2::XMLElement *child = transformElement->FirstChildElement();
      ...
      if (transformType == "translate" &&
          child->QueryFloatAttribute("time", &time) == tinyxml2::XML_SUCCESS) {
        // Align bool
        ...
        for (tinyxml2::XMLElement *pointElement =
        ...
        if (transformType == "translate") {
          ...
          float angleOrTime = 0;
          bool isTime = false;
          if (child->QueryFloatAttribute("time", &time) ==
    ...
    for (tinyxml2::XMLElement *model =
      ...
      if (colorElement) {
        ...
        tinyxml2::XMLElement *diffuseElement =
            colorElement->FirstChildElement("diffuse");
        if (diffuseElement) {
          float r = 0, g = 0, b = 0;
        ...
        tinyxml2::XMLElement *specularElement =
            colorElement->FirstChildElement("specular");
        if (specularElement) {
          float r = 0, g = 0, b = 0;
        ...
        tinyxml2::XMLElement *ambientElement =
            colorElement->FirstChildElement("ambient");
        if (ambientElement) {
          float r = 0, g = 0, b = 0;
        ...
        tinyxml2::XMLElement *emissiveElement =
            colorElement->FirstChildElement("emissive");
        if (emissiveElement) {
          float r = 0, g = 0, b = 0;
        ...
        tinyxml2::XMLElement *shininessElement =
            colorElement->FirstChildElement("shininess");
        if (shininessElement) {
